    ...
    $translateProvider.translations("fr",
      {
        ...
        post_resa_errors: "Les erreurs suivantes se sont produites lors de l'ajout du rendez-vous"
      });
    // messages anglais
    $translateProvider.translations("en",
      {
        ...
        post_resa_errors: "The following errors were met while adding the appointement"
      });
    // langue par défaut
  ...
  .factory('utils', ['config', '$timeout', '$q', function (config, $timeout, $q) {
    ...
    function getErrors(data, $filter) {
      ...
      if (!errors || errors.length == 0) {
        errors = [$filter('translate')('error_unknown')];
      }
  ...
  .controller('rdvMedecinsCtrl', ['$scope', 'utils', 'config', 'dao', '$translate', '$timeout', '$filter', '$locale',
    function ($scope, utils, config, dao, $translate, $timeout, $filter, $locale) {
      ...
      $scope.reserver = function (creneauId) {
        ...
        utils.debug("réservation du créneaupour le client", idClient);
        // attente simulée
